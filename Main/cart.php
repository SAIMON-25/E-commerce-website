    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="text-center mb-4">
                    ...
                    echo "<h4>$name</h4>";
                    echo "<p>Welcome to Computer Island</p>";
                    ?>
                </div>
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="order.php"><i class="fa fa-shopping-cart me-2"></i>My Orders</a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                <!-- Dynamically fatch cart items -->
                                <?php
                                if (isset($_POST['update'])) { // Check if the 'update' button was clicked
                                    if (isset($_POST['qty']) && isset($_POST['product_id'])) { // Check if the required keys exist in the POST data
                                        $qty = $_POST['qty'];
                                        $product_id = $_POST['product_id'];
                                        $ip_add = get_client_ip();

                                        // Update the cart with the new quantity
                                        $sql = "UPDATE cart SET quantity='$qty' WHERE ip_address='$ip_add' AND product_id='$product_id'";
                                        $result = mysqli_query($con, $sql);
                                        if ($result) {
                                ...
                                if (isset($_POST['remove'])) { // Check if the 'remove' button was clicked
                                    if (isset($_POST['product_id'])) { // Check if the product_id exists in POST data
                                        $product_id = $_POST['product_id'];
                                        $ip_add = get_client_ip();

                                        // Delete the product from the cart
                                        $sql = "DELETE FROM cart WHERE ip_address='$ip_add' AND product_id='$product_id'";
                                        $result = mysqli_query($con, $sql);
                                        if ($result) {
